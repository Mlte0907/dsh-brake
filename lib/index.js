...
// 依赖加载：用**动态 import** 而不是顶层 require（2026-09-18 根治）。
//
// 实测失败模式：cordis 加载插件树时并发 import 多个模块，dsh-brake 被加载的同时 dsh-llm
// 可能仍在求值 —— 此时 CJS 的 require(ESM) 抛 "Cannot require() ES Module ... because it
// is not yet fully loaded. ... Try await-ing the import() sequentially in a loop instead"
// （dsh 启动日志原话）。import() 是异步的，会等模块求值完成，不再与加载时序竞争。
//
// 加载窗口内（通常几毫秒）到达的提醒走降级分支，之后自动恢复完整形态；加载彻底失败也
// 不影响插件本体激活 —— 刹车器本身比提醒的形态重要。
let createUserMessage = null
import('@deepseek-ai/dsh-llm')
  .then((mod) => {
    if (typeof mod.createUserMessage === 'function') {
      createUserMessage = mod.createUserMessage
      // 只打一次：运维据它确认"完整形态已生效"（与下面的降级 warn 互斥出现）
      console.log('[dsh-brake] 提醒构建器已就绪（完整形态）')
    }
  })
  .catch((loadError) => {
    console.warn(
      '[dsh-brake] @deepseek-ai/dsh-llm 加载失败，提醒退化为纯文本：'
      + String((loadError && loadError.message) || loadError),
    )
  })
...
function reminder(text, summary) {
  ...
  if (typeof createUserMessage === 'function') return createUserMessage(input)
  // 降级：手工补齐 createUserMessage 会加的字段（role=user + 随机 id + 冻结），
  // 形状与 dsh-llm 的 UserMessage 一致，宿主侧无需感知差异。
